...
.SYNOPSIS
    ...
    Pobiera i instaluje Pythona (je??li brak), Flask, spotdl, yt-dlp i FFmpeg.
    Uruchamia serwer Flask po instalacji.
.DESCRIPTION
    Projekt studencki ??? narz??dzie do pobierania playlist Spotify jako MP3 320kbps.
#>
...
if (-not $PythonExe) {
    ...
    # Od??wie?? PATH w bie????cej sesji
    $env:PATH = [System.Environment]::GetEnvironmentVariable("PATH", "User") + ";" + [System.Environment]::GetEnvironmentVariable("PATH", "Machine")
    
    $PythonExe = Find-Python
    if (-not $PythonExe) {
        Write-Host "  [???] Instalacja Pythona nie powiod??a si??. Zainstaluj r??cznie z https://python.org" -ForegroundColor Red
        exit 1
    }
...
# --- Instalacja zale??no??ci Python ---
Write-Host ""
Write-Host "  [*] Instalowanie zale??no??ci Pythona (Flask, spotdl, yt-dlp)..." -ForegroundColor Cyan
& $PythonExe -m pip install --upgrade pip --quiet
& $PythonExe -m pip install -r requirements.txt --quiet

if ($LASTEXITCODE -eq 0) {
    Write-Host "  [???] Zale??no??ci zainstalowane." -ForegroundColor Green
} else {
    Write-Host "  [???] B????d instalacji zale??no??ci. Sprawd?? po????czenie z internetem." -ForegroundColor Red
    exit 1
}
...
Write-Host "  ??? Instalacja zako??czona!" -ForegroundColor Green
Write-Host ""
Write-Host "  Uruchamiam serwer..." -ForegroundColor Cyan
Write-Host "  Otw??rz przegl??dark??: " -NoNewline
Write-Host "http://localhost:5000" -ForegroundColor Yellow
Write-Host "  ??????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????????" -ForegroundColor DarkGray
Write-Host ""

# Otwieramy przegl??dark?? po 2 sekundach
